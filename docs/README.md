...
number of kids, and accompanying **adults**) → allergies → food choices (pølser vs pizza, lompe vs
pølsebrød, godteposer vs piñata) — or taps **"Hopp over"** into **advanced mode** (the full controls).
Either way they instantly get an age-aware shopping list (food, drink, tableware, decorations) with
quantities rounded up to real Norwegian pack sizes, plus a printable checklist and timeline. After the
wizard the app **leads with the Handleliste**. A **home vs. kindergarten** toggle adjusts for
Helsedirektoratet sugar guidance, allergy **quantity** sliders add the right allergy-safe products, and
the entire goods catalog is **user-editable**. State lives in the URL so a plan can be shared with a
co-parent or the class chat. No login, no cookies, Norwegian-first.
