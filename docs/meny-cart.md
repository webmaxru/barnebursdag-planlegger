...
2. It POSTs `{ items: [{ query, quantity, name }] }` to the managed API (`POST /api/meny/cart`).
3. The managed API (`api/shared/meny.cjs`) searches MENY for each term (concurrency 3, bounded retries,
   and a 30-second global deadline), picks the best in-stock hit with an `ean`, **merges duplicate
   EANs**, and returns full MENY **cart items**
   (`{ ean, quantity, product, pricePerUnit, linePrice, comparePricePerUnit }`) plus
...
The managed API intentionally exposes product resolution only. Shared-cart creation must remain in the
browser.
...
- **Managed API deadline.** Resolution stops after 30 seconds to stay below Static Web Apps' 45-second
  request limit. Completed matches are returned with `partial: true`; if nothing matched, the API
  returns 504 and the user can retry.
...
| Need | File |
|------|------|
| Product resolver | `api/shared/meny.cjs` |
